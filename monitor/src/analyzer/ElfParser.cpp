...
void ElfParser::parse_64() {
    ...
    StorageItem *strtab = item_list[header.e_shstrndx];
    if(!strtab) {
        /* . . . then something is rather wrong. */
        status = false;
        return;
    }
    StorageString *strtab_content = NULL;
    strtab_content = read_content(strtab);
    if(strtab_content == NULL) {
